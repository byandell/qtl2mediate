...
NOTE: sdp_to_pattern not working correctly

```{r}
m <- match(rownames(snp_scan_obj[snp_scan_obj[,pheno_name] >= 2.5,]), snpprobs$snpinfo$snp_id)
table(snpprobs$snpinfo$sdp[m])
table(qtl2mediate:::sdp_to_pattern(snpprobs$snpinfo$sdp[m], snpprobs$haplos))
tmp <- sort(unique(snpprobs$snpinfo$sdp[m]))
qtl2mediate:::sdp_to_pattern(tmp, snpprobs$haplos)
```
