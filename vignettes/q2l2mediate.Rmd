...
```{r}
qtl2ggplot::ggplot_scan1(snp_scan_obj, snpprobs$snpinfo, patterns = "hilit", drop_hilit = 3,
...
```{r}
sdp <- triad_sdp(chr_id, pos_Mbp,
                 scan_window,
                 pheno_name, pheno_data = phe_mx,
...
```{r}
id <- XXX
med_triad <- mediation_triad_qtl2(
  target = phe_mx,
  mediator = med_ls[[1]][, id, drop = FALSE],
  annotation = med_ls[[2]],
  covar_tar = cov_tar,
